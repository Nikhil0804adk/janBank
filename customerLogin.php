...
<div class="login">
  ...
  // If the user isn't logged in, try to log them in
    if (isset($_POST['submit'])) {
      ...
      $user_username = mysqli_real_escape_string($dbc, trim($_POST['unamec']));
      $user_password = mysqli_real_escape_string($dbc, trim($_POST['passwc']));
      if (!empty($user_username) && !empty($user_password)) {
        // Look up the username and password in the database
        $query = "SELECT unamec FROM customer WHERE unamec = '$user_username' AND passwc = '$user_password'"
        ...
        if (mysqli_num_rows($data) == 1) {
          ...
          $_SESSION['unamec'] = $row['unamec'];
          setcookie('unamec', $row['unamec'], time() + (60 * 60 * 24 * 30));  // expires in 30 days
          $home_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/customerhome.php';
